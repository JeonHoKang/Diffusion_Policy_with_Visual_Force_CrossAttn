...
dataset_path = "/home/lm-2023/jeon_team_ws/playback_pose/src/Diffusion_Policy_ICRA/orange_sd_66_imgB.zarr.zip"
    ...
    def __init__(self, train=True):
        ...
        if train:
           ...
           # Save the stats to a file
            with open('stats_orange_force.json', 'w') as f:
...
class DiffusionPolicy_Real_SingleView:     
    def __init__(self, train=True):
        ...
        force_obs_dim = 4
        # observation feature has 514 dims in total per step
        obs_dim = vision_feature_dim + force_obs_dim + lowdim_obs_dim 
        # action dimension should also correspond with the state dimension (x,y,z, x, y, z, w)
        action_dim = 7
        # parameters
        pred_horizon = 16
        ...
        if train:
            ...
            stats = dataset.stats
           # Save the stats to a file
            with open('stats_force_singleview.json', 'w') as f:
                json.dump(stats, f, cls=NumpyEncoder)
                print("stats saved")
            # create dataloader
            dataloader = torch.utils.data.DataLoader(
